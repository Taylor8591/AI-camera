...
static int encoder_init(EncoderCtx *enc, unsigned int width, unsigned int height)
{
    ...
    if (ret) {
        fprintf(stderr, "mpp_buffer_get frm_buf failed ret=%d\n", ret);
        return -1;
    }
...
static void copy_nv12_to_mpp_buffer(EncoderCtx *enc,
                                    const unsigned char *src_y,
                                    const unsigned char *src_uv)
{
    RK_U8 *dst = (RK_U8 *)mpp_buffer_get_ptr(enc->frm_buf);
    RK_U8 *dst_y = dst;
    RK_U8 *dst_uv = dst + enc->hor_stride * enc->ver_stride;
    unsigned int y;

    mpp_buffer_sync_begin(enc->frm_buf);

    for (y = 0; y < enc->height; ++y) {
        memcpy(dst_y + y * enc->hor_stride, src_y + y * g_y_stride, enc->width);
    }

    for (y = 0; y < enc->height / 2; ++y) {
        memcpy(dst_uv + y * enc->hor_stride, src_uv + y * g_uv_stride, enc->width);
    }

    mpp_buffer_sync_end(enc->frm_buf);
}

static int encode_one_frame(EncoderCtx *enc, RK_U32 frame_idx, int *encoded_frames)
{
    MPP_RET ret;
    MppFrame frame = NULL;
...
static void flush_encoder(EncoderCtx *enc)
{
    MppFrame frame = NULL;
    MppPacket packet = NULL;
    int timeout_count = 0;
    ...
    if (mpp_frame_init(&frame) == MPP_OK) {
        ...
        enc->mpi->encode_put_frame(enc->ctx, frame);
        mpp_frame_deinit(&frame);
    }

    while (timeout_count < 10) {
        if (enc->mpi->encode_get_packet(enc->ctx, &packet) != MPP_OK) {
            timeout_count++;
            continue;
        }
...

        {
            ...
            if (len > 0 && g_fifo_fd >= 0 && write_full_packet(ptr, len) < 0) {
                perror("flush write fifo failed");
            }
        }
    ...
    if (!got_packet) {
        printf("encoder flush: no pending packets\n");
    }
}
...
int main(int argc, char *argv[])
{
    ...
    while (g_running) {
        ...
        copy_nv12_to_mpp_buffer(&enc, src_y, src_uv);
        if (encode_one_frame(&enc, (RK_U32)frame_count, &encoded_frames) < 0) {
            xioctl(g_fd, VIDIOC_QBUF, &buf);
            break;
        }
        submitted_frames++;
